...
#
# # Добавляем обработчик консоли к логгеру
...
#     try:
#         testfile = os.path.join(directory, "test_write_access.txt")
#         with open(testfile, "w") as f:
#             f.write("This is a test file to check write access.")
#
#         os.remove(testfile)
#         logging.info(f"Write access to {directory} is OK.")
#     except Exception as e:
#         logging.error(f"No write access to {directory}: {e}")
#
# def main(text: str):
...
#     temp_dir = os.path.join(home_dir, "kensakusensei/files")
#
#     # Проверка записи в директорию
#     check_write_access(temp_dir)
#
...
#         logging.info("MeCab initialized correctly.")
#     except Exception as e:
#         logging.error(f"Failed to initialize MeCab: {e}")
#         return
#
#     rare_words = []
#
...
#
#     logging.info(f"Trying to write results to {output_path}")
#     try:
#         with open(output_path, "w", encoding="utf-8") as f:
#             logging.info(f"Writing results to {output_path}")
#             f.write("Rare Words:\n")
#             f.write(";".join(rare_words) + "\n")
#         logging.info(f"Successfully wrote results to {output_path}")
#     except Exception as e:
